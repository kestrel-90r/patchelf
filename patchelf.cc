...
static FileContents readFile(const std::string & fileName)
{
    ...
    FileContents contents = std::make_shared<std::vector<unsigned char>>(st.st_size);

    #ifdef _MSC_VER
    // Windowsでは、intまたはlongを使用
    long bytesRead = read(fd, contents->data(), contents->size());
    #else
    ssize_t bytesRead = read(fd, contents->data(), contents->size());
    #endif
    
    if (bytesRead == -1)
        error("read");
    if ((size_t) bytesRead != contents->size())
...
// 2つの引数を受け付けるreadFileのオーバーロード関数を追加
static FileContents readFile(const std::string & fileName, size_t maxSize)
{
    struct stat st;
    if (stat(fileName.c_str(), &st) != 0)
        error("stat");
    ...
    // maxSizeが指定されている場合は、ファイルサイズとmaxSizeの小さい方を使用
    // 明示的に型を指定してstd::minを使用
    size_t size = std::min<size_t>(static_cast<size_t>(st.st_size), maxSize);
